...
__aicore__ inline CacheAddresses ResolveCacheAddresses(GM_ADDR *inputList) {
  CacheAddresses addresses;
  addresses.hcBeforeNorm = inputList[17];
  addresses.invRms = inputList[18];
  addresses.sumOut = inputList[19];
  addresses.normOut = inputList[20];
  addresses.mixedInput = inputList[21];
  addresses.rmsRstd = inputList[22];
  return addresses;
}
}  // namespace
...
class KernelWorker
 ...
 public:
  ...
  static constexpr uint32_t TILING_IDX = 24;
  static constexpr uint32_t EVENT_IDX = 10;
  static constexpr uint32_t PROFILE_IDX = 11;
  static constexpr uint32_t WORKSPACE_IDX = 23;
 ...
 private:
  ...
  __aicore__ inline void ExecuteMhcNormCast(const TaskDesc &taskDesc) {
    if ASCEND_IS_AIV {
      ...
      CacheAddresses addresses = ResolveCacheAddresses(input_list);
      int64_t rowBytes = tiling->hcMult * tiling->d * sizeof(float);
